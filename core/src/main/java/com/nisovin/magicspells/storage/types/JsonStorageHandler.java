...
public class JsonStorageHandler implements StorageHandler {
	...
	public void load(Spellbook spellbook) {
		...
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Loading spellbook of UUID '%s' from JSON.", player.getUniqueId())) {
			...
			try (var _ = MagicDebug.section("Loading spellbook from file '%s'.", (Supplier<String>) () -> finalDataFolder.relativize(finalSpellbookPath).toString())) {
				...
				spells:
				try (var _ = MagicDebug.section("Loading spells.")) {
				...
				bindings:
				try (var _ = MagicDebug.section("Loading spell bindings.")) {
					...
					for (String castItemString : bindings.keySet()) {
						try (var _ = MagicDebug.section("Adding bindings for cast item '%s'.", castItemString)) {
							CastItem castItem;
							try (var _ = MagicDebug.section("Parsing cast item '%s'.", castItemString)) {
	...
	private void loadLegacy(Spellbook spellbook, Path path) {
		...
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Loading legacy text file spell book.")) {
			try (Scanner scanner = new Scanner(path.toFile(), StandardCharsets.UTF_8)) {
				while (scanner.hasNext()) {
					...
					try (var _ = MagicDebug.section(spell, "Adding spell '%s'.", data[0])) {
						spellbook.addSpell(spell);
						...
						try (var _ = MagicDebug.section("Adding cast items.")) {
	...
	public void save(Spellbook spellbook) {
		...
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Saving spellbook of UUID '%s' to JSON.", player.getUniqueId())) {
			...
			try (var _ = MagicDebug.section("Saving spells...")){
			...
			try (var _ = MagicDebug.section("Saving custom spell bindings...")) {
				...
				for (Map.Entry<CastItem, ItemBindings> entry : itemBindings.entrySet()) {
					...
					try (var _ = MagicDebug.section("Saving custom spell bindings for cast item '%s'...", item)) {
