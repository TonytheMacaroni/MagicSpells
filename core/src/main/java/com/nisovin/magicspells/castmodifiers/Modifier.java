...
public class Modifier implements IModifier {
	...
	public static Modifier fromString(@NotNull String string) {
		try (var _ = MagicDebug.section(DebugCategory.MODIFIERS, "Resolving modifier '%s'.", string)) {
	...
	private boolean applyEvent(Predicate<IModifier> modifier, BooleanSupplier condition, BooleanPredicate action) {
		try (var _ = MagicDebug.section(DebugCategory.MODIFIERS, "Checking modifier '%s'.", text)) {
			boolean check;
			try (var _ = MagicDebug.section("Checking modifier condition...")) {
	...
	private boolean checkCondition(boolean check) {
		try (var _ = MagicDebug.section(DebugCategory.MODIFIERS, "Performing simplified '%s' modifier action check.", type.name().toLowerCase())) {
			return switch (type) {
				case REQUIRED -> {
