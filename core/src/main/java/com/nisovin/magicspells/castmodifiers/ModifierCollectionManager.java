	...
	private ModifierCollection loadCollection(String internalName) {
		...
		try (var _ = MagicDebug.section(builder -> builder
			...
			try {
				...
				MagicDebug.warn("Invalid modifier collection %s - invalid 'pass-condition' '%s'.", MagicDebug.resolvePath(internalName), passConditionString);
				return null;
			}

			List<String> modifierStrings = section.getStringList("checks");
			if (modifierStrings.isEmpty()) {
				MagicDebug.warn("Invalid modifier collection %s - 'checks' is not defined.", MagicDebug.resolvePath(internalName));
				return null;
			}

			List<ModifierData> modifiers;
			try (var _ = MagicDebug.section("Resolving 'checks'.")
