...
public class ModifierSet {
	...
	private ModifierSet(List<String> data, Spell spell, boolean isFromManaSystem) {
		...
		try (var _ = MagicDebug.section(DebugCategory.MODIFIERS, "Resolving legacy modifier set.")
	...
	public static ModifierSet fromConfig(@Nullable Spell spell, @NotNull ConfigurationSection config, @NotNull String path) {
		try (var _ = MagicDebug.section(DebugCategory.MODIFIERS, "Resolving modifier set '%s'.", MagicDebug.resolveShortPath(config, path))
			.pushPaths(config, path, DebugPath.Type.LIST)
		) {
			...
			return new ModifierSet(getModifierData(spell, modifierStrings), MagicDebug.resolveShortPath(config, path).get());
		}
	}
	...
	private boolean check(LivingEntity caster, SpellData data, Predicate<Modifier> check) {
		...
		try (var _ = MagicDebug.section(DebugCategory.MODIFIERS, "Processing modifier set '%s'.", MagicDebug.resolveShortPath(path))
			.pushPaths(path, DebugPath.Type.LIST)
		) {
			for (ModifierData modifierData : modifiers) {
				try (var _ = MagicDebug.pushListEntry(modifierData.ordinal)) {
