...
public class ModifierSet {
	...
	@ApiStatus.Internal
	public static List<ModifierData> getModifierData(@Nullable Spell spell, @NotNull List<String> modifierStrings) {
		List<ModifierData> modifiers = new ArrayList<>();
		for (int i = 0; i < modifierStrings.size(); i++) {
			String ordinal = String.valueOf(i);

			try (var ignored1 = MagicDebug.pushPath(ordinal, DebugPath.Type.LIST_ENTRY)) {
				...
				modifiers.add(new ModifierData(modifier, ordinal));
			}
		}
	...
	private boolean check(LivingEntity caster, SpellData data, Predicate<Modifier> check) {
		try (var ignored = MagicDebug.section("Processing modifier set '%s'.", MagicDebug.resolveShortPath(path))
			.pushPaths(path, DebugPath.Type.LIST)
		) {
	...
	public record ModifierData(Modifier modifier, String ordinal) {
