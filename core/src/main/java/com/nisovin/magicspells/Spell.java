	...
	protected void initialize() {
		...
		if (rawSharedCooldowns != null) {
			...
			for (String rawSharedCooldown : rawSharedCooldowns) {
				...
				if (data.length != 2) {
					MagicDebug.warn("Invalid shared cooldown '%s' %s - too many or too few arguments.", rawSharedCooldown, MagicDebug.resolvePath());
					continue;
				}

				Spell spell = MagicSpells.getSpellByInternalName(data[0]);
				if (spell == null) {
					MagicDebug.warn("Invalid spell '%s' in shared cooldown '%s' %s.", data[0], rawSharedCooldown, MagicDebug.resolvePath());
					continue;
				}
				...
				try {
					...
					MagicDebug.warn("Invalid cooldown '%s' in shared cooldown '%s' %s.", data[1], rawSharedCooldown, MagicDebug.resolvePath());
					continue;
				}

				Float old = sharedCooldowns.put(spell, cd);
				if (old != null)
					MagicDebug.warn("Duplicate shared cooldown '%s' %s.", spell.internalName, MagicDebug.resolvePath());
			}
