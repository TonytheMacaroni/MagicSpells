...
public abstract class Spell implements Comparable<Spell>, Debuggable, Listener {
	...
	protected void initializeModifiers() {
		...
		if (locationModifierStrings != null && !locationModifierStrings.isEmpty()) {
			try (var ignored = MagicDebug.section("Initializing 'location-modifiers'...")
				...
				locationModifiers = new ModifierSet(modifierStrings, this);
	...
	protected void initialize() {
		...
		if (rawSharedCooldowns != null) {
			...
			for (String rawSharedCooldown : rawSharedCooldowns) {
				String[] data = rawSharedCooldown.split(" ");
				if (data.length != 2) {
					MagicDebug.warn("Invalid shared cooldown '%s' %s - too many or too few arguments.", rawSharedCooldown, MagicDebug.resolveFullPath());
					continue;
				}

				Spell spell = MagicSpells.getSpellByInternalName(data[0]);
				if (spell == null) {
					MagicDebug.warn("Invalid spell '%s' in shared cooldown '%s' %s.", data[0], rawSharedCooldown, MagicDebug.resolveFullPath());
					continue;
				}

				float cd;
				try {
					cd = Float.parseFloat(data[1]);
				} catch (NumberFormatException e) {
					MagicDebug.warn("Invalid cooldown '%s' in shared cooldown '%s' %s.", data[1], rawSharedCooldown, MagicDebug.resolveFullPath());
					continue;
				}

				Float old = sharedCooldowns.put(spell, cd);
				if (old != null)
					MagicDebug.warn("Duplicate shared cooldown '%s' %s.", spell.internalName, MagicDebug.resolveFullPath());
			}
