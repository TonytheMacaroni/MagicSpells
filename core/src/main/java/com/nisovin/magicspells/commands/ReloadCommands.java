...
public class ReloadCommands {
	...
	private static void reload(CommandContext<CommandSourceStack> context) {
		try (var _ = MagicDebug.section("Reloading plugin.")) {
			MagicSpells plugin = MagicSpells.getInstance();

			try (var _ = MagicDebug.section("Unloading plugin.")) {
				plugin.unload();
			}

			MagicSpells.plugin = plugin;

			try (var _ = MagicDebug.section("Loading plugin.")) {
				plugin.load();
			}
	...
	private static void reloadSpellbook(CommandContext<CommandSourceStack> context) {
		...
		try (var _ = MagicDebug.section("Reloading spellbooks for the selected players.")) {
	...
	private static void reloadEffectLib(CommandContext<CommandSourceStack> context) {
		...
		context.sender().getSender().sendMessage(Component.text("EffectLib reloaded.", MagicSpells.getTextColor()));
	}
