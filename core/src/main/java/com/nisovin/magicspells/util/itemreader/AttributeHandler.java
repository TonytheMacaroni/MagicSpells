...
public class AttributeHandler extends ItemHandler {
	...
	public static LinkedHashMultimap<Attribute, AttributeModifier> getAttributeModifiers(@NotNull List<?> modifierData, @NotNull String name, @Nullable String spellName) {
		try (var _ = MagicDebug.section("Resolving attribute modifier list from '%s'.", name)
			...
			for (int i = 0; i < modifierData.size(); i++) {
				try (var _ = MagicDebug.pushListEntry(i)) {
					...
					switch (value) {
						case String string -> {
							try (var _ = MagicDebug.section("Resolving string-based attribute modifier '%s'.", string)) {
						...
						case Map<?, ?> map -> {
							try (var _ = MagicDebug.section("Resolving section-based attribute modifier.")) {
