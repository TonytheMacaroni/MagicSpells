			...
			for (Map.Entry<String, JsonElement> entry : object.entrySet()) {
				...
				switch (key.toLowerCase()) {
					...
					case "enchants", "enchantments" -> {
						Map<String, Integer> rawEnchants = gson.fromJson(value.getAsJsonObject().toString(), new TypeToken<HashMap<String, Integer>>() {}.getType());
					...
					case "patterns" -> {
						...
						for (JsonElement element : patternStrings) {
							...
							PatternType patternType = PatternType.valueOf(pattern[0]);
