...
public class EntityData {
	...
	public EntityData(@NotNull ConfigurationSection config, @Nullable EntityType forceType, boolean forceOptional) {
		try (var _ = MagicDebug.section(builder -> builder
			...
			// Attributable
			// TODO: Use converter for this instead
			...
			if (attributeModifiers != null && !attributeModifiers.isEmpty()) {
				transformers.put(Attributable.class, (Attributable entity, SpellData _) -> {
			...
			List<ConfigData<PotionEffect>> potionEffects = Util.getPotionEffects(config.getList("potion-effects"));
	...
	private <T> T suppressIfNotType(Class<? extends Entity> entityClass, Supplier<T> supplier) {
		...
		try (var _ = MagicDebug.downgradeWarnings()) {
			return supplier.get();
		}
	}
