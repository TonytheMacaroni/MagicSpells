...
public class MagicItemData implements Cloneable {
	...
	public static class MagicItemAttributes {
		...
		public static final MagicItemAttribute<Multimap<Attribute, AttributeModifier>> ATTRIBUTES = new MagicItemAttribute<>("attributes") {
			...
			private static final Hash.Strategy<AttributeModifier> STRATEGY = new Hash.Strategy<>() {

				@Override
				public int hashCode(AttributeModifier modifier) {
					return Objects.hash(
						modifier.getName(),
						modifier.getAmount(),
				...
				@Override
				public boolean equals(AttributeModifier a, AttributeModifier b) {
			...
			public boolean compare(@NotNull MagicItemData data, @NotNull Multimap<Attribute, AttributeModifier> primary, @NotNull Multimap<Attribute, AttributeModifier> secondary) {
				Set<Attribute> keysSelf = primary.keySet();
				Set<Attribute> keysOther = secondary.keySet();
				if (!keysSelf.equals(keysOther)) return false;


				for (Attribute attribute : keysSelf) {
					Collection<AttributeModifier> modsSelf = primary.get(attribute);
					Collection<AttributeModifier> modsOther = secondary.get(attribute);

					int size = modsSelf.size();
					if (size != modsOther.size()) return false;

					Object2IntOpenCustomHashMap<AttributeModifier> counts = new Object2IntOpenCustomHashMap<>(size, STRATEGY);
					counts.defaultReturnValue(0);

					for (AttributeModifier modifier : modsSelf)
						counts.addTo(modifier, 1);

					for (AttributeModifier modifier : modsOther)
						if (counts.addTo(modifier, -1) == 0)
							return false;
				}
