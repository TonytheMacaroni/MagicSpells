...
public class VariableModSet {
	...
	public static VariableModSet fromConfig(@NotNull ConfigurationSection config, @NotNull String path) {
		try (var _ = MagicDebug.section(DebugCategory.VARIABLE_MODIFIERS, "Processing variable modifier set '%s'.", MagicDebug.resolveShortPath(path))
			...
			for (int i = 0; i < modifierStrings.size(); i++) {
				...
				try (var _ = MagicDebug.section("Processing variable modifier '%s'.", modifierString)
					...
					try {
						...
						MagicDebug.warn("Invalid variable modifier '%s' %s.", modifierString, MagicDebug.resolveFullPath(path));
					}
				}
			}

			return new VariableModSet(modifiers, MagicDebug.resolveShortPath(config, path).get());
		}
	}
	...
	public void process(@NotNull VariableOwner owner, @NotNull SpellData data) {
		try (var _ = MagicDebug.section(DebugCategory.VARIABLE_MODIFIERS, "Processing variable modifier set '%s'.", MagicDebug.resolveShortPath(path))
			...
			for (VariableModData modifier : modifiers) {
				try (var _ = MagicDebug.section("Processing variable modifier '%s'.", modifier.text)
