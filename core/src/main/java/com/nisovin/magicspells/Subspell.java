...
public class Subspell {
	...
	public Subspell(@NotNull String subspell) {
		try (var _ = MagicDebug.section("Resolving subspell '%s'.", subspell)) {
			...
			for (String arg : arguments) {
				try (var _ = MagicDebug.section("Resolving cast argument '%s'.", arg)) {
	...
	private SpellCastResult cast(@NotNull SpellData data) {
		...
		MagicSpells.scheduleDelayedTask(() -> {
			try (var _ = MagicDebug.section(DebugCategory.CAST, "Casting delayed subspell '%s' (mode: %s, targeting: NONE).", text, mode)) {
				castReal(finalData);
			}
		}, delay);
	...
	private SpellCastResult castAtEntity(@NotNull SpellData data, boolean passTargeting) {
		...
		MagicSpells.scheduleDelayedTask(() -> {
			try (var _ = MagicDebug.section(DebugCategory.CAST, "Casting delayed subspell '%s' (mode: %s, targeting: ENTITY).", text, mode)) {
	...
	private SpellCastResult castAtLocation(@NotNull SpellData data) {
		...
		MagicSpells.scheduleDelayedTask(() -> {
			try (var _ = MagicDebug.section(DebugCategory.CAST, "Casting delayed subspell '%s' (mode: %s, targeting: LOCATION).", text, mode)) {
