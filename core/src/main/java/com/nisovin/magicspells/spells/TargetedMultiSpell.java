...
public final class TargetedMultiSpell extends TargetedSpell implements TargetedEntitySpell, TargetedLocationSpell, TargetedEntityFromLocationSpell {
	...
	private CastResult runSpells(SpellData data) {
		...
		if (castRandomSpellInstead.get(data)) {
			...
			boolean casted = action instanceof SpellAction(Subspell spell) && spell.subcast(data).success();
		...
		DelayedSpells delayedSpells = new DelayedSpells(data, stopOnFail, passTargeting, stopOnSuccess);
		int totalDelay = 0;
	...
	private static class DelayedSpells {

		private final IntList tasks;
		private final SpellData data;
		private final boolean stopOnFail;
		private final boolean stopOnSuccess;
		private final boolean passTargeting;

		private DelayedSpells(SpellData data, boolean stopOnFail, boolean passTargeting, boolean stopOnSuccess) {
			this.data = data;
			this.stopOnFail = stopOnFail;
			this.passTargeting = passTargeting;
			this.stopOnSuccess = stopOnSuccess;

			tasks = new IntArrayList();
		}
		...
		private void scheduleSpell(Subspell subspell, int delay) {
			int taskId = MagicSpells.scheduleDelayedTask(() -> {
				if (!data.isValid()) {
					cancel();
					return;
				}
