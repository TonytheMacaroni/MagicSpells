...
public class PassiveSpell extends Spell {
	...
	public void initializeListeners() {
		...
		int count = 0;
		try (var _ = MagicDebug.section("Processing 'triggers'.")
	...
	public boolean activate(SpellData data) {
		try (var _ = MagicDebug.section(DebugCategory.CAST, this, "Activating passive spell '%s'.", this)) {
			...
			MagicSpells.scheduleDelayedTask(() -> {
				try (var _ = MagicDebug.section(DebugCategory.CAST, this, "Activating delayed passive spell '%s'.", this)) {
					activateSpells(data);
				}
			}, delay);
	...
	private boolean activateSpells(SpellData data) {
		...
		disabled = true;
		try (var _ = MagicDebug.section(DebugCategory.CAST, "Casting passive spell '%s' with caster '%s'.", this, data.caster())) {
			MagicDebug.info("Spell data: %s", data);
			...
			try (var _ = MagicDebug.section("Casting subspells.")) {
