...
public class MagicDebug {
	...
	public static Section downgradeWarnings() {
		return new Section.Builder().downgradeWarnings().build();
	}
	...
	private static void pushPaths(@NotNull ArrayDeque<DebugPath> paths, @Nullable ConfigurationSection config, @NotNull String pathString, @NotNull DebugPath.Type type, boolean concrete) {
		Preconditions.checkNotNull(pathString, "Path cannot be null");

		pathString = MagicDebug.resolveShortPath(config, pathString).get();

		String[] pathStrings = pathString.split("\\.");
		for (int i = 0; i < pathStrings.length - 1; i++)
			MagicDebug.pushPath(paths, new DebugPath(pathStrings[i], DebugPath.Type.SECTION, concrete));

		MagicDebug.pushPath(paths, new DebugPath(pathStrings[pathStrings.length - 1], type, concrete));
	}
	...
	public static Supplier<String> resolvePath(@NotNull String subPath) {
		return resolvePath(null, subPath);
	}
	...
	public static Supplier<String> resolvePath(@Nullable ConfigurationSection config, @NotNull String subPath) {
		return () -> {
			...
			// TODO: Shouldn't this pass in the config?
	...
	public record Section(
		...
		public Section pushPaths(@NotNull String path, @NotNull DebugPath.Type type) {
			MagicDebug.pushPaths(paths, null, path, type, type != DebugPath.Type.FILE);
			return this;
		}

		public Section pushPaths(@NotNull String path, @NotNull DebugPath.Type type, boolean concrete) {
			MagicDebug.pushPaths(paths, null, path, type, concrete);
			return this;
		}

		public Section pushPaths(@Nullable ConfigurationSection config, @NotNull String path, @NotNull DebugPath.Type type) {
			MagicDebug.pushPaths(paths, config, path, type, type != DebugPath.Type.FILE);
			return this;
		}

		public Section pushPaths(@Nullable ConfigurationSection config, @NotNull String path, @NotNull DebugPath.Type type, boolean concrete) {
			MagicDebug.pushPaths(paths, config, path, type, concrete);
			return this;
		}
		...
		public static class Builder {
			...
			public Builder paths(@Nullable ConfigurationSection config, @NotNull String path, @NotNull DebugPath.Type type) {
				MagicDebug.pushPaths(paths, config, path, type, type != DebugPath.Type.FILE);
				return this;
			}

			public Builder paths(@Nullable ConfigurationSection config, @NotNull String path, @NotNull DebugPath.Type type, boolean concrete) {
				MagicDebug.pushPaths(paths, config, path, type, concrete);
				return this;
			}
			...
			public Builder downgradeWarnings() {
				this.downgradeWarnings = true;
				return this;
			}
