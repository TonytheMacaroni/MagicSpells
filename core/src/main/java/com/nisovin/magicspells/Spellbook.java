...
// TODO: Check if changes related to can-cast-with-item persisted
public class Spellbook {
	...
	public void load() {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Loading spellbook for player '%s'.", player.getName())) {
	...
	public void save() {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Saving spellbook for player '%s'.", player.getName())) {
			MagicSpells.getStorageHandler().save(this);
		}
	}

	public void reload() {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Reloading spellbook for player '%s'.", player.getName())) {
			removeAllSpells();
			load();
			save();
		}
	...
	public void addGrantedSpells() {
		...
		if (grantAllByDefault || player.isOp() && MagicSpells.grantOpsAllSpells()) {
			try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Granting all spells - %s.", grantAllByDefault ? "all spells are granted by default" : "player is opped")) {
				boolean added = false;

				for (Spell spell : MagicSpells.getSpellsOrdered()) {
					try (var _ = MagicDebug.section(spell, "Attempting to grant spell '%s'.", spell.getInternalName())) {
		...
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Adding granted spells.")) {
			boolean added = false;
	...
	public boolean canLearn(Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Checking if player '%s' can learn spell '%s'.", player.getName(), spell.getInternalName())) {
			if (spell.isHelperSpell()) {
	...
	public boolean canCast(Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Checking if '%s' can cast spell '%s'.", player.getName(), spell.getInternalName())) {
	...
	public boolean canTeach(Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Checking if '%s' can teach spell '%s'.", player.getName(), spell.getInternalName())) {
			if (spell.isHelperSpell()) {
	...
	public boolean hasAdvancedPerm(String spell) {
		...
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Checking if '%s' has the advanced permission '%s'.", player.getName(), permission)) {
	...
	public void addSpell(@NotNull Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Adding spell '%s' to spellbook for player '%s'.", spell.getInternalName(), player.getName())) {
			spells.add(spell);
	...
	public void addCustomBinding(@NotNull CastItem item, @NotNull Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Adding a custom binding for spell '%s' to cast item '%s'.", spell.getInternalName(), item)) {
	...
	public boolean removeCustomBinding(@NotNull CastItem item, @NotNull Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Removing a custom binding for spell '%s' from cast item '%s'.", spell.getInternalName(), item)) {
	...
	public void removeCustomBindings(@NotNull CastItem item) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Removing custom bindings from cast item '%s'.", item)) {
			ItemBindings bindings = itemBindings.get(item);
	...
	private void loadDefaultBindings() {
		...
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Adding default bindings to spellbook.")) {
			Multimap<CastItem, Spell> defaultBindings = HashMultimap.create();
			for (Spell spell : spells) {
				try (var _ = MagicDebug.section(spell, "Checking cast items for spell '%s'.", spell.getInternalName())) {
	...
	public void removeSpell(Spell spell) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Removing spell '%s' from spellbook of player '%s'.", spell.getInternalName(), player.getName())) {
			spells.remove(spell);
	...
	public boolean hasSpell(Spell spell, boolean checkGranted) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, spell, "Checking if player '%s' has spell '%s'.", player.getName(), spell.getInternalName())) {
	...
	public Spell nextSpell(@Nullable ItemStack item) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Retrieving the next selected spell for player '%s'.", player.getName())) {
	...
	public Spell prevSpell(@Nullable ItemStack item) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Retrieving the previous selected spell for player '%s'.", player.getName())) {
			CastItem castItem = item == null || item.isEmpty() ? CastItem.AIR : new CastItem(item);
			MagicDebug.info("Checking bindings for cast item '%s'.", castItem);
	...
	public Spell getActiveSpell(@Nullable ItemStack item) {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Retrieving currently selected spell for player '%s'.", player.getName())) {
			CastItem castItem = item == null || item.isEmpty() ? CastItem.AIR : new CastItem(item);
	...
	public void removeAllSpells() {
		try (var _ = MagicDebug.section(DebugCategory.SPELLBOOK, "Removing all spells from spellbook of player '%s'.", player.getName())) {
			for (Spell spell : spells) {
				spell.unloadPlayerEffectTracker(player);
				if (spell instanceof BuffSpell buffSpell) buffSpell.turnOff(player);
			}
